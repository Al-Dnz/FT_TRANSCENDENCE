<template>
  <div class="flex flex-row h-full w-full">
    <div class="flex flex-col h-full w-3/4 pt-3 pl-4 pr-4 divide-y-2">
      <!-- Header -->
    ...
    <div class="w-1/4">
      <ul class="list-none">
        <li v-for="user in conv?.userList" v-bind:key="user.id">
          <div class="flex flex-row pt-8">
            <img :src="getImgUrl(user.pic)" class="w-10 h-10 rounded-full" />
            <div class="pl-2">
              <div class="font-bold">
                {{ user.name }}
              </div>
